...
void overlay (SDL_Surface* image_surface, SDL_Surface* logo)
{
    ...
    for (int i = wl; i > 0; i--)
    {
        for (int j = hl; j > 0; j--)
        {
            Uint32 pixel = get_pixel(logo, i, j);
            Uint8 r, g, b;
    ...
    if (hl==wl)
    {
        for (int i = 0; i < wi; i++)
        {
            for (int j = 0; j < hi; j++)
            {
                ...
                Uint8 r, g, b;
                SDL_GetRGB(pixel, image_surface->format, &r, &g, &b);
                if (c!=0 && (i >= wi/2.7 && i <= wi-wi/2.7) &&(j>=hi/2.7 && j <= hi-hi/2.7))
                {
                    r = red[c];
                    g = green[c];
                    b = blu[c];
                    Uint32 _pixel = SDL_MapRGB(image_surface->format, r, g, b);
                    put_pixel(image_surface, i, j, _pixel);
                    c--;
                }            
            }
        }     
...
void res(SDL_Surface* logo, int x, int y)
{
    ...
    for(int i = 0; i<w; i++)
    {
        for(int j = 0; j<h; j++)
        {
            Uint32 pixel = get_pixel(logo, i*(w/x), j*(h/y));
            Uint8 r, g, b;
            Uint8 red, green, blu;
            SDL_GetRGB(pixel, logo->format, &r, &g, &b);
            red = r;
