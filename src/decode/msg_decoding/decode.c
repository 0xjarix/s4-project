...
{
    char* dst = calloc((unsigned int)len, sizeof(char));
...
char* convertEpi(char* input, size_t max_size) {
    ...
    buffer[max_size] = '\0';
    for(size_t es = 0, k = 0; (input[es] != '\0') && k < max_size; ++es,\
            k += 2) {
        if(input[es] == 'w') {
            buffer[k] = '0';
            buffer[k + 1] = '0';
...
char* decode(char* input, int version, int level, int print)
{
    //printf("Input : %s\n", input);
    /*
    ...
    for(size_t g = 0; g < DCR->size; ++g)
    {
        for(size_t b = 0; b < DCR->group[g]->size; ++b)
        {
            ...
            {
                if (print)
                    printf("Encoded word %2li: %s\n", w + 1,\
                            DCR->group[g]->blocks[b]->words[w]);
            }
            for(size_t w = 0; w <\
                    (size_t)ECC_CODEWORDS_PER_BLOCK[level][version]; ++w)
            {
                if (print)
                    printf("Correction word %2li: %s\n", w + 1,\
                            DCR->group[g]->blocks[b]->correction[w]);
            }
        }
    }
