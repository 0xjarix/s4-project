 ...
 * (nsym), computes the
 syndromes polynomial. Mathematically, it's essentially equivalent to a
 Fourrier Transform
 (Chien search being the inverse).*/

struct Array* rs_calc_syndromes(struct Array *msg, uint8_t nsym,
...
/*Returns true if the message + ecc has no error of false otherwise (may not
 * always catch a wrong decoding or a wrong message,particularly if there are
 * too many errors, but it usually does)*/
...
struct Array* rs_find_errdata_locator(struct Array *e_pos, struct gf_tables
        *gf_table)
{
 ...
 * error/erasures/errata locator Sigma*/
struct Array* rs_find_error_evaluator(struct Array *synd,
        struct Array *err_loc, uint8_t nsym, struct gf_tables *gf_table)
{
    struct Array *res = malloc(sizeof(struct Array));
    ...
    for(size_t i = 0; i < X->used; i++){
        ...
        for(size_t k = 0; k < err_loc_prime_tmp->used; k++)
            err_loc_prime = gf_mul(err_loc_prime,
                    err_loc_prime_tmp->array[k], gf_table);
...
struct Array* rs_find_error_locator(struct Array* synd, uint8_t nsym,
        uint8_t erase_count, struct gf_tables *gf_table)
{
    struct Array *err_loc = malloc(sizeof(struct Array));
    ...
    for(int i = 0; i < nsym-erase_count;i++){
        ...
        for(size_t j = 1 ; j < err_loc->used; j++){
            delta ^= gf_mul(err_loc->array[err_loc->used - (j+1)],
                    synd->array[K - j], gf_table);
        }
        ...
        if(delta != 0){
            if(old_loc->used > err_loc->used){
                ...
                old_loc = gf_poly_scale(err_loc,
                        gf_inverse(delta, gf_table), gf_table);
...
struct Array* rs_find_errors(struct Array *err_loc, size_t nmess,
        struct gf_tables *gf_table)//nmess is len(msg_in)
{
    size_t errs = err_loc->used - 1;
    size_t counter = 0;
...
struct Array* rs_forney_syndromes(struct Array *synd, struct Array *pos,
        uint8_t nmess, struct gf_tables *gf_table){
    ...
    for (size_t i = 0; i < pos->used; i++) {
        ...
        for (size_t j = 0; j < fsynd->used - 1; j++)
            fsynd->array[j] = gf_mul(fsynd->array[j], x, gf_table)
                ^ fsynd->array[j + 1];
    }
    return fsynd;
}

struct Array* rs_correct_msg(struct Array *msg_in, uint8_t nsym,
        struct Array *erase_pos, struct gf_tables *gf_table){
    if (msg_in->used > 255) {
    ...
    struct Array *fsynd = rs_forney_syndromes(synd, erase_pos,
            msg_out->used, gf_table);
    struct Array *err_loc = rs_find_error_locator(fsynd, nsym, 0, gf_table);
    struct Array *err_pos = rs_find_errors(reverse_arr(err_loc),
            msg_out->used, gf_table);
    if (err_pos == NULL) {
